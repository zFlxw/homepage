<div class="w-full min-h-screen text-center flex flex-col justify-center items-center">
	<div class="relative inline-block text-center">
		<h1 class="text-5xl md:text-7xl font-semibold mt-6 md:mt-12 text-center block cool-hr">
			Legal Notice
		</h1>
	</div>
	<div class="text-2xl mt-16">
		<h2 class="text-4xl font-semibold">
			Angaben gem&auml;&szlig; &sect; 5 TMG / <br />Information pursuant to sect. 5 German Telemedia
			Act (TMG)
		</h2>
		<p class="mt-6 text-3xl">
			Maik Zimmerschied<br />
			Lindenstr. 2<br />
			54531 Wallscheid
		</p>

		<h2 class="text-4xl font-semibold mt-24">Kontakt / Contact</h2>
		<p class="mt-6">E-Mail: lxcalflxw@gmail.com</p>

		<p class="mt-24 text-center">
			<strong>Quelle / Source</strong>:
			<a href="https://www.e-recht24.de/impressum-generator.html" class="text-primary"
				>https://www.e-recht24.de/impressum-generator.html</a
			>
		</p>
	</div>
</div>
